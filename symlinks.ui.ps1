...
$TokenFilterElementTextBox_KeyUp = {
    ...
    if (($e.KeyCode -eq [Keys]::Enter -or $e.KeyCode -eq [Keys]::Return) -and $sdr.Focused -and $sdr.ReadOnly -eq $false) {
        if ($null -eq $sdr.Text -or $sdr.Text -eq $sdr.Tag) { $sdr.Text = $sdr.Tag }
            ...
            <#
                TODO: Improve node filter tokenization.
                instead of just returning invalid filter on text with paths,
                check if the path is a valid Tag within TreeView's Nodes
                if valid, check the TreeView Node and deploy the tokenized input
                as a new panel
            #>
